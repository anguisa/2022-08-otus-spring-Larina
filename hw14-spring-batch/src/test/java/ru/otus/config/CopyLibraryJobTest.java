...
public class CopyLibraryJobTest {
    ...
    @Test
    void testMongoToH2JobExitStatus() throws Exception {
    ...
    @Test
    void testMongoToH2JobResults() throws Exception {
        List<Author> expectedAuthors = Stream.of(AUTHOR_1, AUTHOR_2).sorted(Comparator.comparing(Author::getName)).collect(Collectors.toList());
        List<Genre> expectedGenres = Stream.of(GENRE_1, GENRE_2).sorted(Comparator.comparing(Genre::getTitle)).collect(Collectors.toList());
        List<Book> expectedBooks = Stream.of(BOOK_1, BOOK_2, BOOK_3).sorted(Comparator.comparing(Book::getTitle))
            .map(book -> new Book(book.getId(), book.getTitle(), book.getAuthor(),
                book.getGenres().stream().sorted(Comparator.comparing(Genre::getTitle)).collect(Collectors.toList()),
                book.getComments().stream().sorted(Comparator.comparing(Comment::getText)).collect(Collectors.toList()))
            )
        ...
        List<Author> authors = jdbc.query("select id, name from author order by name", new AuthorMapper());
        ...
        List<Genre> genres = jdbc.query("select id, title from genre order by title", new GenreMapper());
                ...
                .thenComparing((o1, o2) -> {
                    ...
                    if (result == 0) {
                        for (int i = 0; i < o1.getComments().size(); i++) {
                            result = o1.getComments().get(i).getText().compareTo(o2.getComments().get(i).getText());
                            if (result != 0) {
                                break;
                            }
                        }
    ...
    private static class BookCommentMapper implements RowMapper<Book> {

        private final Map<String, Book> booksMap;

        public BookCommentMapper(Map<String, Book> booksMap) {
