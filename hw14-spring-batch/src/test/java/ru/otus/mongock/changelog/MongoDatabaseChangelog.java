...
public class MongoDatabaseChangelog {
    ...
    public void initAuthors(AuthorDao authorDao) {
        AUTHOR_1 = authorDao.save(new Author(null, "Катя Петрова"));
        AUTHOR_2 = authorDao.save(new Author(null, "Женя Максимова"));
    }

    @ChangeSet(order = "003", id = "initGenres", author = "olga")
    public void initGenres(GenreDao genreDao) {
        GENRE_1 = genreDao.save(new Genre(null, "Детектив"));
        GENRE_2 = genreDao.save(new Genre(null, "Фантастика"));
    }

    @ChangeSet(order = "004", id = "initComments", author = "olga")
    public void initComments(CommentDao commentDao) {
        COMMENT_1 = commentDao.save(new Comment(null, "Интересная"));
        COMMENT_2 = commentDao.save(new Comment(null, "Увлекательная"));
        COMMENT_3 = commentDao.save(new Comment(null, "Скучная"));
        COMMENT_4 = commentDao.save(new Comment(null, "Захватывающая"));
        COMMENT_5 = commentDao.save(new Comment(null, "Необычная"));
    }

    @ChangeSet(order = "005", id = "initBooks", author = "olga")
    public void initBooks(BookDao bookDao) {
        BOOK_1 = bookDao.save(new Book(null, "Мой детектив", AUTHOR_1, List.of(GENRE_1), List.of(COMMENT_1, COMMENT_2)));
        BOOK_2 = bookDao.save(new Book(null, "Необычная фантастика", AUTHOR_2, List.of(GENRE_2), List.of(COMMENT_3)));
        BOOK_3 = bookDao.save(new Book(null, "Странная сказка", AUTHOR_2, List.of(GENRE_1, GENRE_2), List.of(COMMENT_4, COMMENT_5)));
    }
}