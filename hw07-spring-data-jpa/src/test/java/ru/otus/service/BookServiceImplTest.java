...
public class BookServiceImplTest {
    ...
    public void setUp() {
        ...
        when(authorDao.findById(EXPECTED_AUTHOR.getId())).thenReturn(Optional.of(EXPECTED_AUTHOR));
        ...
        when(genreDao.findById(EXPECTED_GENRE.getId())).thenReturn(Optional.of(EXPECTED_GENRE));
        when(genreDao.findByTitle(EXPECTED_GENRE.getTitle())).thenReturn(Optional.of(EXPECTED_GENRE));
        when(genreDao.findByTitleIn(List.of(EXPECTED_GENRE.getTitle()))).thenReturn(List.of(EXPECTED_GENRE));
    }
    ...
    void shouldUpdateBook() {
        ...
        when(authorDao.findByName(authorNew.getName())).thenReturn(Optional.empty());
        when(authorDao.save(any())).thenReturn(authorNew);

        Genre genreNew = new Genre(2L, "My genre " + randomTxt);
        when(genreDao.findByTitle(genreNew.getTitle())).thenReturn(Optional.empty());
